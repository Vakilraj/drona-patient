...
class MedicineDetails extends React.Component {
	constructor(props) {
		...
		if (Array.isArray(props.navigation.state.params.item)) {
			...
			medicineDosasesType.forEach((val, index) => {
			...
			console.log('====>>> medicineDosasesType <<<===', JSON.stringify(medicineDosasesType));
		}
		this.state = {
			noteData: '',
	...
	SearchFilterFunctionDosage = (text, index) => {
		...
		if (text && text.length > 0) {
			if (Validator.isNumberHyphanDotSlashValidate(text)) {
				...
				temp.forEach((val) => {
					temp[index].dosageValue = text
				})
				this.setState({taperedData: temp, dosageDropdownArr: searchResult.length > 0 ? searchResult : text , showStateDosage: searchResult.length > 0 ? true : false  });
				prvLength = text.length;
			} else {
				this.setState({ dosageSearchTxt: '', dosageDropdownArr: doasagesPatterArr });
			}
	...
	clickOnDosege = (item, index) => {
		this.setState({ dosageSearchTxt: '', showStateDosage: true, selectedIndex: index })
	}
		...
		return (
			<View>
				...
				<DropDownPicker
					...
					textStyle={{ fontFamily: CustomFont.fontName, color: Color.black, fontSize: CustomFont.font16 }}
					itemStyle={{
						justifyContent: 'flex-start'
					}}
					defaultValue={item.unitTextValue}
					dropDownStyle={{ backgroundColor: '#fafafa', zIndex: 4 }}
					onChangeItem={item => {
						const temp = [...this.state.taperedData];
						temp.forEach((_item, _index) => {
							temp[index].unitTextValue = item.value;
						})
						this.setState({ taperedData: temp })
					}}
				...
				<KeyboardAvoidingView behavior={Platform.OS === "ios" ? 'padding' : null}>
					...
					<View style={{ margin: responsiveWidth(5) }}>
						...
						<FlatList
							...
							scrollEnabled={true}
							style={{ maxHeight: responsiveFontSize(75) }}
