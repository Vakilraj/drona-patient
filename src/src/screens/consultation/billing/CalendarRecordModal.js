...
class CalendarRecordModal extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
	...
	async componentDidMount() {
		this.setState({selectedDate:Moment(new Date).format('DD/MM/YYYY')})
	...
	clickOnDone = () => {
		...
		if (this.state.isModalVisibleSingle) {
			this.setState({ isModalVisibleSingle: false, selectedDate: this.state.showHeaderDate });
		...
		return (
			<View>
				<TouchableOpacity style={{ flexDirection: 'row', borderRadius: 4, borderColor: Color.createInputBorder, alignItems: 'center', borderWidth: 1, marginTop: responsiveHeight(.6), height: responsiveHeight(6), backgroundColor: Color.white }}
					...
					<View style={{ flex: 7 }}>
						<Text style={{ fontFamily: CustomFont.fontName, fontSize: CustomFont.font14, color: Color.feeText, opacity: 1, marginLeft: responsiveWidth(5) }}>{this.state.selectedDate}</Text>
					</View>
					<View style={{}}>
						<Image source={CalendarImage} style={{height:responsiveFontSize(2),width:responsiveFontSize(2), marginRight:responsiveWidth(5), }} />
					</View>
									...
									onDayPress={day => {
										try {
											if (counter % 2 == 0) {
												...
												this.setState({ selectedDateArr: { [startDateSelection]: { startingDay: true, color: Color.primaryBlue, textColor: 'white' } } })
											} else {
						...
						<ScrollView>
							<View style={{ flex: 1, flexDirection: 'column' }}>
								...
								<Calendar
									...
									hideDayNames={true}
									//minDate={this.props.paymentDate}
									maxDate={new Date()}
								/>
